...
/>`
    ...
    if (relationshipType === 'many-to-one' || (relationshipType === 'one-to-one' && ownerSide === true)) {
        return `${relation.otherEntityName}Id: this.get${relation.otherEntityNameCapitalizedPlural}()${
            index !== total - 1 ? ',' : ''
        }`;
    }
    if (relationshipType === 'many-to-many' && ownerSide === true) {
        return `${pluralize(relation.relationshipNamePlural)}: t.list(this.get${relation.otherEntityNameCapitalizedPlural}())${
            index !== total - 1 ? ',' : ''
        }`;
    }
    return '';
