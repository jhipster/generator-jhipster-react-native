...
const expoPackageJson = require('../generators/app/templates/package.expo.json');

const expoCliVersion = expoPackageJson['expo-cli-version'];
...
function generateReactNativeApp() {
  ...
  try {
    ...
    const generateCommand = `expo-cli@${expoCliVersion} init --npm -t expo-template-blank-typescript --no-install --non-interactive ${name}`;
