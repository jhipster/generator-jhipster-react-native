...
function patchNavigationForEntity(name) {
    ...
    // import entity screens to navigation
    const navigationDeclaration = `{ name: '${name}Entity', route: '${this.kebabCaseName}', component: ${name}EntityScreen, options: { title: '${this.pluralName}', headerLeft: () => <HeaderBackButton onPress={() => navigationRef.current.navigate('Entities')} /> }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
        insert: navigationDeclaration,
    });
    const navigationDeclarationDetail = `{ name: '${name}EntityDetail', route: '${this.kebabCaseName}/detail', component: ${name}EntityDetailScreen, options: { title: 'View ${this.name}', headerLeft: () => <HeaderBackButton onPress={() => navigationRef.current.navigate('${name}Entity')} /> }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
        insert: navigationDeclarationDetail,
    });
    const navigationDeclarationEdit = `{ name: '${name}EntityEdit', route: '${this.kebabCaseName}/edit', component: ${name}EntityEditScreen, options: { title: 'Edit ${this.name}', headerLeft: () => <HeaderBackButton onPress={() => navigationRef.current.navigate('${name}EntityDetail')} /> }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
