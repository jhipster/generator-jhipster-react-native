...
function patchNavigationForEntity(name) {
    ...
    // import entity screens to navigation
    const navigationDeclaration = `{ name: '${name}Entity', route: '${this.kebabCaseName}', component: ${name}EntityScreen, options: { title: '${this.pluralName}' }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
        insert: navigationDeclaration,
    });
    const navigationDeclarationDetail = `{ name: '${name}EntityDetail', route: '${this.kebabCaseName}/detail', component: ${name}EntityDetailScreen, options: { title: 'View ${this.name}' }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
        insert: navigationDeclarationDetail,
    });
    const navigationDeclarationEdit = `{ name: '${name}EntityEdit', route: '${this.kebabCaseName}/edit', component: ${name}EntityEditScreen, options: { title: 'Edit ${this.name}' }},`;
    this.patchInFile(entityStackPath, {
        before: 'jhipster-react-native-navigation-declaration-needle',
