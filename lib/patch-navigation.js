...
function patchNavigationForEntity(name) {
    ...
    // import entity screens to navigation
    const navigationDeclaration = `{ name: '${name}Entity', route: '${this.kebabCaseName}', component: ${name}EntityScreen, options: { title: '${this.pluralName}', headerLeft: () => <HeaderBackButton onPress={() => navigate('Entities')} />, headerRight: () => <HeaderBackButton onPress={() => navigate('FieldTestEntityEntityEdit')} backImage={(props) => <Ionicons name="md-add-circle-outline" size={32} {...props} />} /> }},`;
    this.patchInFile(entityStackPath, {
