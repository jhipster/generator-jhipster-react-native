function patchEntityApi() {
  ...
  const get${this.context.entityNamePlural} = (options) => api.get('${microservicePath}api/${this.context.entityApiUrl}', options)
  ...
  get${this.context.entityNamePlural}: () => {
    return {
      ok: true,
      data: require('../../shared/fixtures/get-${this.context.entityPluralFileName}.json'),
    }
  },
  ...
  delete${this.context.entityNameCapitalized}: (${this.context.entityInstance}Id) => {
    ...
    let sagaConnections = `
    takeLatest(${this.context.entityNameCapitalized}Types.${this.context.entityTableName.toUpperCase()}_REQUEST, get${this.context.entityNameCapitalized}, api),
    takeLatest(${this.context.entityNameCapitalized}Types.${this.context.entityTableName.toUpperCase()}_ALL_REQUEST, get${this.context.entityNamePlural}, api),
    takeLatest(${this.context.entityNameCapitalized}Types.${this.context.entityTableName.toUpperCase()}_UPDATE_REQUEST, update${this.context.entityNameCapitalized}, api),
    takeLatest(${this.context.entityNameCapitalized}Types.${this.context.entityTableName.toUpperCase()}_DELETE_REQUEST, delete${this.context.entityNameCapitalized}, api),`;
  ...
  search${this.context.entityNamePlural}: (query) => {
    return {
      ok: true,
      data: require('../../shared/fixtures/search-${this.context.entityNamePlural.toLowerCase()}.json')
    }
  },`;
    ...
    search${this.context.entityNamePlural},`;
        sagaConnections += `
    takeLatest(${this.context.entityNameCapitalized}Types.${this.context.entityTableName.toUpperCase()}_SEARCH_REQUEST, search${this.context.entityNamePlural}, api),`;
    }
    ...
    this.patchInFile(sagaIndexFilePath, {
        ...
        insert: `import { get${this.context.entityNameCapitalized}, get${this.context.entityNamePlural}, update${this.context.entityNameCapitalized}, delete${this.context.entityNameCapitalized}${
            this.context.searchEngine ? `, search${this.context.entityNamePlural}` : ''
        } } from '../../modules/entities/${this.context.entityFileName}/${this.context.entityFileName}.sagas'`,
    });
