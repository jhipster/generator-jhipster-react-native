function patchEntityApi(entity) {
  ...
  const updateMethod = entity.useOldPutMappingCode
  ...
  getAll${entity.entityNamePlural}: () => {
    return {
      ok: true,
      data: require('../../shared/fixtures/get-all-${entity.entityPluralFileName}.json'),
    };
  },
  ...
  if (entity.searchEngine) {
    console.log(`SearchEngine: ${entity.searchEngine}`);
    apiMethods += `
  ...
  search${entity.entityNamePlural}: (query) => {
    return {
      ok: true,
      data: require('../../shared/fixtures/search-${entity.entityPluralFileName}.json'),
    }
  },`;
